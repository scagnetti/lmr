...
class ExtractStockPriceDevOneYearTest < ActiveSupport::TestCase
  ...
  test "the stock price dev one year calculation on DAX shares" do
    exec_on_index_shares("DE0008469008"){|share|
      figure = StockPriceDevHalfYear.new
      begin
        e = OnVistaExtractor.new(share)
        e.extract_stock_price_dev_one_year(figure)
  ...
  test "the stock price dev one year calculation on DOW shares" do
