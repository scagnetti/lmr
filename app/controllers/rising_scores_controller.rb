class RisingScoresController < ApplicationController
  ...
  def index
    @rising_scores = RisingScore.joins('INNER JOIN shares ON shares.isin = rising_scores.isin').where("rising_scores.created_at >= ?", Time.zone.now.beginning_of_day).order("rising_scores.days DESC")
